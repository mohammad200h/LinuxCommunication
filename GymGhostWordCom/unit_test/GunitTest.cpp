...
TEST(Server_Test,add_client_to_queue_and_remove_client_from_queue){
    ...
    //id
    gym_state.key.id =boost::uuids::random_generator()();
    server->add_client_to_queue(gym_state.key.id,socket);
    /*
    the id should be stored as it is encountered for
    the first time
    */
    EXPECT_TRUE(server->clients_queue.size()==1);
    /*
    ...
    */
    ...
    EXPECT_TRUE(server->clients_queue.size()==0);
...
TEST(Server_Test,getClientsInQueue){
    Server* server = new Server();
    ...
    thread server_worker(start_Server,server);
    
    std::string path = std::experimental::filesystem::current_path();
    cout<<"current path::"<<path<<endl;
    
    while(!server->serverIsReady); 
    bp::child c("./build/client");
    
    while (c.running() ){
        while(!server-> is_there_any_client_in_queue());
        vector <boost::uuids::uuid> ids = server->getClientsInQueue();
        
        GymworldState gymState = server->getGymStateforId(ids[0]);
        ghost_state.key.id = ids[0];
        server->send(ids[0],ghost_state);
    ...
    server_worker.join();
    // EXPECT_TRUE(server->clients_queue.size()==0)<<"clients_id::len:: "<<server->clients_queue.size()<<endl;
